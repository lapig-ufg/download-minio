...
from fastapi import APIRouter, HTTPException, Query, Path,Response, Request
...
async def get_geofile(
    ...
    with MongoClient(settings.MONGODB_URL) as client:
        ...
        if filterLabel is None:
            match type:
                case "json":
                    return Response(jsonable_encoder(dumps({'urls':[f'{base_url}']})),200, media_type='application/json')
                case _:
                    return Response(f'{base_url}',200, media_type='text/plain')
            ...
            match type:
                case "json":
                    return Response(jsonable_encoder(dumps({'urls':rows})),200, media_type='application/json')
                case "wget":
                    comand = '#/bin/bash\n'
                    for _filter in filters:
